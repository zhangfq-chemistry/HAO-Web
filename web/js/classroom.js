...
function setScreen(name) {
  ...
  for (const button of el.classroomScreenTabs ?? []) {
    ...
    if (button.dataset.classroomScreen === "live") button.disabled = !state.startedAt;
    if (button.dataset.classroomScreen === "review") button.disabled = !state.completed;
  }
...
async function startActivity() {
  ...
  state.step = 0;
  state.prediction.claim = el.classroomPredictionClaim?.value || currentDefinition().claimOptions?.[0]?.[0] || "";
  state.visitedViews = [];
  state.evidence = [];
  state.explanation = "";
  state.transfer = "";
  ...
  if (el.classroomTransfer) el.classroomTransfer.value = "";
  if (el.classroomSaveState) el.classroomSaveState.textContent = "等待猜想";
  configureTemplateUI();
  updateTimerText();
  setScreen("live");
  updateStep(0);
...
function updateStep(index) {
  ...
  state.step = [0, 1, 3].includes(index) ? index : index < 1 ? 0 : index < 3 ? 1 : 3;
  const definition = templateDefinitions[state.template];
  const phaseCopy = {
    0: definition.tasks[0],
    1: ["观察并选择证据", `${definition.tasks[1][1]} 完成观察后，只选最能支持判断的证据。`],
    3: ["用证据写出结论", `${definition.tasks[3][1]} 迁移到另一个轨道是可选的延伸思考。`],
  };
  const [title, text] = phaseCopy[state.step];
  ...
  for (const panel of el.classroomStepPanels ?? []) {
    const panelStep = Number(panel.dataset.classroomStepPanel);
    const active = panelStep === state.step || (state.step === 1 && panelStep === 2);
    panel.classList.toggle("active", active);
  }
  const phaseNumber = state.step === 0 ? 1 : state.step === 1 ? 2 : 3;
  if (el.classroomStepBadge) el.classroomStepBadge.textContent = `第 ${phaseNumber} 步 / 3 步`;
  ...
  if (el.classroomNextButton) {
    el.classroomNextButton.textContent = state.step === 0 ? "锁定猜想并观察" : state.step === 1 ? "证据选好了" : "完成并查看总结";
  }
  if (state.step === 1 && previousStep < 1 && state.visitedViews.length === 0) {
    const firstView = currentDefinition().observation?.[0]?.[0] || "orbital";
    markViewVisited(firstView);
    setLinkedView(firstView);
  }
  renderObservationRoute();
  updateStageVisibility();
  updateEvidencePanel();
  if (state.step === 3) buildConclusion();
  requestAnimationFrame(() => {
    resizeSketchCanvas();
    resizeClassroomViews();
  });
...
function savePrediction() {
  state.prediction = {
    claim: el.classroomPredictionClaim?.value || state.prediction.claim,
  ...
  if (el.classroomSaveState) el.classroomSaveState.textContent = "猜想已锁定";
  updateEvidencePanel();
}
...
function validateStep() {
  ...
  if (state.step === 3) {
    ...
    state.transfer = el.classroomTransfer?.value.trim() || "";
  }
  return true;
}
...
function nextStep() {
  ...
  updateStep(state.step === 0 ? 1 : 3);
}

function previousStep() {
  if (state.step === 3) updateStep(1);
  else if (state.step === 1) updateStep(0);
}
...
function updateLearningTrail() {
  const evidence = state.evidence.length ? state.evidence : selectedEvidence();
  if (el.classroomTrailPrediction) {
    el.classroomTrailPrediction.textContent = state.predictionSaved
      ? predictionSummary()
      : state.startedAt ? "尚未锁定" : "尚未提交";
  }
  if (el.classroomTrailObservation) {
    el.classroomTrailObservation.textContent = state.visitedViews.length
      ? state.visitedViews.map((view) => evidenceLabels[view]).join(" → ")
      : "尚未开始探究";
  }
  if (el.classroomTrailEvidence) {
    el.classroomTrailEvidence.textContent = evidence.length ? evidence.map((key) => evidenceLabels[key] || key).join("、") : "尚未选择";
  }
  if (el.classroomTrailExplanation) el.classroomTrailExplanation.textContent = state.explanation || el.classroomExplanation?.value.trim() || "尚未形成";
  if (el.classroomTrailTransfer) el.classroomTrailTransfer.textContent = state.transfer || el.classroomTransfer?.value.trim() || "尚未完成";
  for (const item of el.classroomTrailItems ?? []) {
    const step = Number(item.dataset.classroomTrailStep);
    item.classList.toggle("active", step === state.step);
    item.classList.toggle("complete", step < state.step || (state.completed && step === 3));
  }
}
...
function buildDiagnosticFeedback() {
  ...
  const messages = [];
  const fields = new Set(currentDefinition().predictionFields ?? []);
  if (fields.has("claim") && state.prediction.claim !== expectedClaim()) {
    messages.push(`核心猜想需要修正：${claimLabel(expectedClaim())}。`);
  }
  if (fields.has("radial") && state.prediction.radial !== radial) {
    messages.push(`径向节点应为 ${radial} 个；请回到 R(r) 的非原点零点核对。`);
  }
  if (fields.has("angular") && state.prediction.angular !== angular) {
    messages.push(`角向节点应为 ${angular} 个；请在角向函数中核对零值方向。`);
  }
  if (fields.has("axis") && axis !== "复合方向" && axis !== "无特定方向" && state.prediction.axis !== axis) {
    messages.push(`主要方向应为 ${axis}；请把角向波瓣与坐标轴对齐后再判断。`);
  }
  if (!messages.length) messages.push("猜想与观察结果一致。现在不要重复描述图像，请解释所选证据为什么能够支持它。");
  setFeedback(messages.join(" "), messages.length === 1 && messages[0].startsWith("猜想") ? "success" : "warning");
}
...
function buildConclusion() {
  ...
  if (el.classroomConclusion) {
    const conclusions = {
      links: `${target?.label} 的四幅图都来自 Ψ=R(r)Y(θ,φ)：三维轨道显示等值面，径向图读取距离变化，角向图读取方向变化，投影读取选定平面上的 Ψ。`,
      nodes: `${target?.label} 有 ${radial} 个径向节点和 ${angular} 个角向节点；前者来自 R(r) 的非原点零点，后者来自 Y(θ,φ) 的零值方向。`,
      slice: `${target?.label} 的二维截面是在选定平面上逐点计算 Ψ，不是把三维外壳压扁；颜色保留相位，零值线对应节点。`,
      basics: `${target?.label} 的主要方向为 ${axis}；两种颜色表示波函数正、负相位，不表示电荷或电子自旋。`,
      compare: `${target?.label} 与 ${comparisonTarget()?.label} 的差异应分别追溯到 R(r) 与 Y(θ,φ)，而不是只凭外形大小判断。`,
    };
    el.classroomConclusion.textContent = conclusions[state.template];
  }
  updateLearningTrail();
}
...
function finishActivity(openReview = true) {
  ...
  state.transfer = el.classroomTransfer?.value.trim() || state.transfer;
  state.evidence = selectedEvidence();
  buildDiagnosticFeedback();
  updateReviewScreen();

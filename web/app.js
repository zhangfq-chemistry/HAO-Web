...
function resetRadialCamera(radius = 4.5) {
  currentRadialRadius = radius;
  radialCamera.position.set(0, 0, radius * 1.5);
  radialCamera.lookAt(0, 0, 0);
  updateRadialFrustum(radius);
  radialControls.target.set(0, 0, 0);
...
function updateRelationRadiusMax(options) {
  ...
  relationRadiusMax = surfaceRadius > 0.001 ? surfaceRadius * 1.02 : options.radius * 0.86;
}
...
function addRelationOrbital(state) {
  const group = new THREE.Group();
  if (state.r > 0.001) {
    group.add(sphereWireLines(state.r, relationColors.radius, 0.72));
  }
  const arrowLength = Math.max(state.r, state.maxRadius * 0.22, 1);
  group.add(relationArrow(state.direction, arrowLength, relationColors.direction, arrowLength * 0.11, arrowLength * 0.035));
  const point = markerSphere(Math.max(0.08, state.maxRadius * 0.018), relationColors.point);
  point.position.copy(state.point);
  group.add(point);
  scene.add(group);
...
function addRelationRadial(state) {
  ...
  const currentPercent = state.r / Math.max(0.0001, radialRelationMeta.radialExtent);

  if (radialSurfaceObject) {
    const rings = radialSurfaceObject.userData.rings;
    const currentRings = Math.max(1, Math.round(currentPercent * rings));
    radialSurfaceObject.geometry.setDrawRange(0, currentRings * 128 * 6);
  }
  if (radialSideObject) {
    const rings = radialSideObject.userData.rings;
    const currentRings = Math.max(1, Math.round(currentPercent * rings));
    const sideAngles = radialSideObject.userData.sideAngles;
    radialSideObject.geometry.setDrawRange(0, currentRings * sideAngles * 6);
  }
  if (radialCurveObject) {
    ...
    // solidCurveFromPoints produces a TubeGeometry with 8 radial segments
    // curve has 'samples' points on positive side, 'samples' on negative side.
    const tubularSegments = radialCurveObject.geometry.parameters.tubularSegments;
    const segmentsPerSample = tubularSegments / totalPoints;
    const centerSegment = tubularSegments / 2;
    const visibleSegmentsRight = currentSamples * segmentsPerSample;
    const visibleSegmentsLeft = currentSamples * segmentsPerSample;
    // We only want to show the center portion (from -r to +r)
    const startSegment = Math.max(0, Math.floor(centerSegment - visibleSegmentsLeft));
    const countSegments = Math.ceil(visibleSegmentsRight + visibleSegmentsLeft);
    radialCurveObject.geometry.setDrawRange(startSegment * 8 * 6, countSegments * 8 * 6);
  }
  ...
  group.add(lineFromPoints(
    [new THREE.Vector3(x, 0, radialRelationMeta.baseZ - 0.15), new THREE.Vector3(x, 0, radialRelationMeta.heightTop)],
    relationColors.radius,
    0.95
  ));
  const point = markerSphere(0.07, relationColors.point);
...
function addRadialNodeMarkers(group, params, options, meta) {
  ...
  for (const nodeR of nodes) {
    ...
    if (options.radialCurve) {
      const value = radialValueForMode(params, nodeR, options.radialMode);
      const z = curveZ + (value / meta.maxAbs) * meta.heightScale;
      for (const sign of [-1, 1]) {
...
function renderRadialView(params, options) {
  ...
  const radialExtent = relationEnabled()
    ? Math.max(0.001, relationRadiusMax || radialDisplayExtent(params))
    : Math.max(5, radialDisplayExtent(params));
  const values = [];
  ...
  const heightScale = radiusScale * 0.43;
  const baseZ = -1.25;
  const diskZ = baseZ;
  const curveLift = 0.08;
  const sideZ = diskZ + radiusScale * 0.72;
  ...
  const halfSphere = new THREE.Mesh(
    new THREE.SphereGeometry(radiusScale, 128, 48, 0, Math.PI * 2, Math.PI / 2, Math.PI / 2),
    new THREE.MeshPhongMaterial({
      color: 0xb8b8b8,
      side: THREE.DoubleSide,
      transparent: true,
  ...
  if (options.radialSide) {
    const connector = new THREE.Mesh(
      new THREE.CylinderGeometry(radiusScale, radiusScale, Math.max(0.1, sideZ - diskZ), 160, 1, true),
    ...
    group.add(connector);
    group.add(circleLine(radiusScale, sideZ, 0xffffff, 0.12));
    group.add(circleLine(radiusScale, diskZ, 0xffffff, 0.08));
  ...
  if (options.radialAxis) {
    ...
    radialAxisObject = makeRadialAxisGroup(radiusScale + 0.9, radiusScale + 0.45, axisZ, options.radialMode);
...
for (const input of [
  ...
  input.addEventListener("input", () => {
    ...
    if ([el.syncRotation, el.scanRadius, el.scanTheta, el.scanPhi, el.scanSliceZ].includes(input)) {
